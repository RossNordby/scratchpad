...
using HeadlessTests23;
using System.Diagnostics;
using System.Numerics;
using System.Runtime.CompilerServices;
using System.Runtime.InteropServices;

namespace HeadlessTests23.DemoStyle.Dancers;
...
public class DemoDancers
{
    ...
    public DemoDancers Initialize<TNarrowPhaseCallbacks, TCollisionFilter>(
        ...
        IThreadDispatcher threadDispatcher, BufferPool pool, (int iterationCount, int substepCount) dancerSolveDescription, DressUpDancer<TCollisionFilter> dressUpDancer, TCollisionFilter filterForDancerBodies)
        ...
        where TCollisionFilter : unmanaged
    {
        ...
        hipShape.ComputeInertia(1f, out var hipInertia);
        SourceBodyHandles.Hips = mainSimulation.Bodies.Add(BodyDescription.CreateDynamic(new RigidPose(hipsPosition, hipOrientation), hipInertia, new(mainSimulation.Shapes.Add(hipShape), float.MaxValue), new(0.01f)));
        ...
        abdomenShape.ComputeInertia(1f, out var abdomenInertia);
        SourceBodyHandles.Abdomen = mainSimulation.Bodies.Add(BodyDescription.CreateDynamic(new RigidPose(abdomenPosition, abdomenOrientation), abdomenInertia, new(mainSimulation.Shapes.Add(abdomenShape), float.MaxValue), new(0.01f)));
        ...
        chestShape.ComputeInertia(1f, out var chestInertia);
        SourceBodyHandles.Chest = mainSimulation.Bodies.Add(BodyDescription.CreateDynamic(new RigidPose(chestPosition, chestOrientation), chestInertia, new(mainSimulation.Shapes.Add(chestShape), float.MaxValue), new(0.01f)));
        ...
        headShape.ComputeInertia(1f, out var headInertia);
        SourceBodyHandles.Head = mainSimulation.Bodies.Add(BodyDescription.CreateDynamic(headPosition, headInertia, new(mainSimulation.Shapes.Add(headShape), float.MaxValue), new(1e-2f)));
        ...
        upperLegShape.ComputeInertia(1, out var upperLegInertia);
        lowerLegShape.ComputeInertia(1, out var lowerLegInertia);
        var upperLegDescription = BodyDescription.CreateDynamic(new RigidPose(upperLegPosition, upperLegOrientation), upperLegInertia, new(mainSimulation.Shapes.Add(upperLegShape), float.MaxValue), new(0.01f));
        var lowerLegDescription = BodyDescription.CreateDynamic(new RigidPose(lowerLegPosition, lowerLegOrientation), lowerLegInertia, new(mainSimulation.Shapes.Add(lowerLegShape), float.MaxValue), new(0.01f));
        ...
        upperArmShape.ComputeInertia(1, out var upperArmInertia);
        lowerArmShape.ComputeInertia(1, out var lowerArmInertia);
        var upperArmDescription = BodyDescription.CreateDynamic(new RigidPose(upperArmPosition, upperArmOrientation), upperArmInertia, new(mainSimulation.Shapes.Add(upperArmShape), float.MaxValue), new(0.01f));
        var lowerArmDescription = BodyDescription.CreateDynamic(new RigidPose(lowerArmPosition, lowerArmOrientation), lowerArmInertia, new(mainSimulation.Shapes.Add(lowerArmShape), float.MaxValue), new(0.01f));
        ...
        mainSimulation.Solver.Add(SourceBodyHandles.Hips, new OneBodyAngularServo { ServoSettings = ServoSettings.Default, SpringSettings = new SpringSettings(30, 1), TargetOrientation = mainSimulation.Bodies.GetBodyReference(SourceBodyHandles.Hips).Pose.Orientation });
        ...
        mainSimulation.Statics.Add(new(new Vector3(0, -1.24f, 0), new(mainSimulation.Shapes.Add(new Box(1000, 1, 1000)), 0.1f)));
        ...
        {
            sourceSimulation.Bodies.GetDescription(sourceHandle, out var description);
        ...
        for (int i = 0; i < Handles.Length; ++i)
        {
            ...
            var narrowPhaseCallbacks = default(TNarrowPhaseCallbacks).Create(dancerFilters, new PairMaterialProperties { FrictionCoefficient = 0.4f, MaximumRecoveryVelocity = 20, SpringSettings = new SpringSettings(120, 1) }, levelOfDetail <= 0.5f ? 3 : int.MaxValue);
            var dancerSimulation = Simulation.Create(new BufferPool(16384), narrowPhaseCallbacks,
                new DemoPoseIntegratorCallbacks(new Vector3(0, -10, 0)), new SubsteppingTimestepper(dancerSolveDescription.substepCount), dancerSolveDescription.iterationCount,
                ...
                initialAllocationSizes: new SimulationAllocationSizes { Bodies = 128, Statics = 1, Islands = 1, ShapesPerType = 8, Constraints = 512, ConstraintsPerTypeBatch = 64, ConstraintCountPerBodyEstimate = 8 });
    ...
    {
        var poseA = simulation.Bodies.GetBodyReference(a).Pose;
        var poseB = simulation.Bodies.GetBodyReference(b).Pose;
        ...
        int groupIndex, int limbSubgroupIndexStart)
    {
        ...
        var bodyPose = simulation.Bodies.GetBodyReference(body).Pose;
        var upperPose = simulation.Bodies.GetBodyReference(upperLimb).Pose;
        var lowerPose = simulation.Bodies.GetBodyReference(lowerLimb).Pose;
    ...
    static void ConstrainOrientation(Simulation simulation, BodyHandle a, BodyHandle b)
    {
        simulation.Solver.Add(a, b, new AngularServo
        {
            ...
            TargetRelativeRotationLocalA = Quaternion.Concatenate(simulation.Bodies.GetBodyReference(b).Pose.Orientation, Quaternion.Conjugate(simulation.Bodies.GetBodyReference(a).Pose.Orientation))
        });
    }
    ...
    unsafe void ExecuteDancer(int dancerIndex, int workerIndex)
    {
        ...
        {
            var targetBody = dancerSimulation.Bodies.GetBodyReference(targetHandleBuffer[j]);
            ref var source = ref MotionHistory[historicalStateStartIndex + j];
            targetBody.Pose = new RigidPose(source.Pose.Position + GetOffsetForDancer(dancerIndex, DancerGridWidth), source.Pose.Orientation);
            targetBody.Velocity = source.Velocity;
        }
    ...
    public unsafe void UpdateTargets(Simulation mainSimulation)
    {
        ...
        for (int i = 0; i < sourceHandleBuffer.Length; ++i)
        {
            var body = mainSimulation.Bodies.GetBodyReference(sourceHandleBuffer[i]);
            MotionHistory.EnqueueUnsafely(new MotionState { Pose = body.Pose, Velocity = body.Velocity });
        }
