...
unsafe struct RopeNarrowPhaseCallbacks : INarrowPhaseCallbacks
{
    ...
    public RopeNarrowPhaseCallbacks(CollidableProperty<RopeFilter> filters, int minimumDistanceForCollisions = 3) : this(filters, new PairMaterialProperties { FrictionCoefficient = 1, MaximumRecoveryVelocity = 2, SpringSettings = new SpringSettings(30, 1) }, minimumDistanceForCollisions)
    {
    }
...
public class RopeTwistVideoDemo : Demo
{
    public static BodyHandle[] BuildRopeBodies(Simulation simulation, in Vector3 start, int bodyCount, float bodySize, float bodySpacing, float massPerBody, float inverseInertiaScale)
    {
        ...
        {
            Activity = new(.01f),
            Collidable = new(ropeShapeIndex, float.MaxValue)
        };
    ...
    public unsafe override void Initialize(int threadCount)
    {
        ...
        Simulation = Simulation.Create(BufferPool,
            new RopeNarrowPhaseCallbacks(filters, new PairMaterialProperties { FrictionCoefficient = 1.0f, MaximumRecoveryVelocity = float.MaxValue, SpringSettings = new SpringSettings(1200, 1) }),
            new DemoPoseIntegratorCallbacks(new Vector3(0, -10, 0)), new SubsteppingTimestepper(60), 1);
        ...
        for (int twistIndex = 0; twistIndex < 10; ++twistIndex)
        {
            ...
            var description = BodyDescription.CreateDynamic(wreckingBallPosition, bigWreckingBallInertia, new (bigWreckingBallIndex, float.MaxValue), new (0.01f));
        ...
        Simulation.Statics.Add(new StaticDescription(new Vector3(0, 0, 0), new (Simulation.Shapes.Add(new Box(200, 1, 200)), 0.1f)));
    }
}
