...
public class RagdollDemo
{
    public static float MassMultiplier = 1f;
    public static float SleepThreshold = .01f;
    static BodyHandle AddBody<TShape>(TShape shape, float mass, in RigidPose pose, Simulation simulation) where TShape : unmanaged, IConvexShape
    {
        ...
        shape.ComputeInertia(mass, out var shapeInertia);
        var description = BodyDescription.CreateDynamic(pose, shapeInertia, new (shapeIndex, float.MaxValue), new (0.01f));
