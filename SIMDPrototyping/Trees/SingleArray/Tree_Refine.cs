...
namespace SIMDPrototyping.Trees.SingleArray
{
    partial class Tree
    {
        ...
        public unsafe void AgglomerativeRefine(int nodeIndex, out bool nodesInvalidated)
        {
            ...
            for (int i = 0; i < subtrees.Count; ++i)
            {
                ...
                else
                {
                    ...
                    if (leaf->NodeIndex >= nodeCount)
                        Console.WriteLine("baD");
                    var parentNode = nodes + leaf->NodeIndex;
                    if (leaf->ChildIndex >= parentNode->ChildCount)
                        Console.WriteLine("bad");
            ...
            while (remainingNodesCount >= 2)
            {
                ...
                for (int i = 0; i < remainingNodesCount; ++i)
                {
                    for (int j = i + 1; j < remainingNodesCount; ++j)
                    {
                        ...
                        if (nodeIndexA >= tempNodeCount || nodeIndexB >= tempNodeCount)
                            Console.WriteLine("baD");
                        BoundingBox merged;
        ...
        {
            if (tempNodeIndex >= tempNodeCount)
                Console.WriteLine("bad");
